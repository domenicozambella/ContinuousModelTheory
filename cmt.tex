...
We say that $f:M\to N$, a partial map, is \emph{$\mathds{I}$-elementary\/} if for every $\varphi(x)\in\mathds{I}$ and every $a\in({\rm dom }f)^{|x|}$
...
\begin{lemma}
  Let $\mu$ be a $R$-valued ${\EuScript B}$-measure.
  Then there is ${}^*\!\mu\in{}^*\!F$ such that 
  
  \ceq{\hfill {\rm I}\big( f{\cdot}{}^*\!\mu\big)}{=}{\int_\Omega f\,{\rm d}\mu}
  
  for every ${\EuScript B}$-measurable $R$-valued functions $f$.
\end{lemma}
...
The context will suggest if symbol $0$ denotes the zero of $M$, $F$ or $R$.

The $\mathds{I}$-formula $\forall^{\sf F}\!f\ge0\ {\rm I}(f{\cdot}\mu^+)\ge0$ is obviously true in ${\EuScript M}$ and so is the same formula for $\mu^-$.
