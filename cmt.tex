...
\begin{lemma}
  Let  ${\EuScript M}$ be a small model. 
  Then the following are equivalent
  \begin{itemize}
...
\begin{theorem}
  ...
  Let ${\EuScript M}$ be an e.c.\@ Cauchy complete model.
  Then  every consistent formula $\varphi(x)\in\mathds{H}(M)$ is contained in a Cauchy type over $M$ of small cardinality.
\end{theorem}

\begin{proof}
  If $\varphi(x)$ is $\mathds{I}$-atomic then, as ${\EuScript M}$ is e.c., it is realized by some $a\in M^{|x|}$ hence we can take the type $\{x{=}a,\, \varphi(x)\}$. 
  Induction is immediate.
\end{proof}
