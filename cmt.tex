...
In this case the language $L_{\sf M}$ governs the structure $\langle{\sf M}_1\dots,{\sf M}_k\rangle$.
...
We will see that $\mathds{I}$ and $\mathds{H}$-elementarity between saturated models coincide ${\EuScript L}$-elementarity, hence the converse of (1) holds trivially.

Now we introduce a two of weak notions of elementarity that are invertible in all models.
We say that $f:M\to N$ is an \emph{partial $\mathds{I}$-embedding\/} if (1) holds when $\varphi(x)$ is $\mathds{I}$-atomic.
...
As $C$ range over the compact neighborhoods of the diagonal $\Delta\subseteq R^2$ and $t(x,z)$ over the parameter-free terms, these formulas define a system of entougages on $U^{|x|}$.
We refer to the topology associated as the \emph{${\sf R}$-topology.}
It is not difficult to verify that ${\sf R}$-topology on $U^{|x|}$ coincide with the product of the ${\sf R}$-topology on $U$.
We define the type

\ceq{\hfill\emph{$x\sim_{\sf R}y$}}{=}{\Big\{\tau_{t,C}(x,y)\ :\ t=t(x,z)\textrm{ a term, } C\textrm{ a compact neigborhood of }\Delta\Big\}.}

In other words, this type says that $\forallM z\ \big[t(x,z)=t(y,z)\big]$ holds for every term $t(x,z)$.
Again, it is easy to verify that if $|x|=|y|=\lambda$ then $x\sim_{\sf R}y$ then if and only if $x_i\sim_{\sf R}y_i$ for every $i<\lambda$.
...
We say that $p(x)$ is a \emph{Cauchy type\/} if it is consistent, ${\sf R}$-invariant, and $p(x)\cup p(y)\rightarrow x\sim_{\sf R} y$.

\begin{fact}
  Let ${\EuScript M}$ be a model that is $\lambda$-$\mathds{I}$-saturated for some $\lambda>|{\EuScript L}|$.
  Then every Cauchy type $p(x)\subseteq\mathds{I}(M)$ is realized in $M$.
\end{fact}
...
\begin{proof}
  ...
  Clearly $b\sim_{\sf R}a$.
  By invariance, $b\models p(x)$. 
\end{proof}
...
\begin{proposition}\label{prop_sim_approx}
  ...
  For every given $\varphi'>\varphi$ there are some $a_i\models\varphi(x)$, some terms $t_i=t_i(x,z)$, and some compact $C_i\subseteq R^2\smallsetminus\Delta$ such that 

  \ceq{\hfill\varphi(x)}{\rightarrow}{\bigwedge_{i=1,\dots,n}\tilde\tau_{t_i,C_i}(a_i,x)\ \ \rightarrow\ \ \varphi'(x),}
...
\begin{proof}
  ...
  \ceq{\hfill\neg\varphi(x)}{\leftrightarrow}{\bigvee_{a\,\models\neg\varphi(x)}\big\{\tau_{t,\Delta}(a,x)\ :\ t=t(z,z)\textrm{ a term}\big\}.}
  ...
  \ceq{\hfill\varphi(x)}{\leftrightarrow}{\bigwedge_{\varphi(x)\rightarrow\tilde\tau_{t,C}(a,x)}\tilde\tau_{t,C}(a,x).}
...
Let $L_{\sf M}$ contain symbols for all continuous functions $M^n\to M$.
...
Therefore the definition of Cauchy sequence and the corresponding notion of completeness make perfectly sense in ${\EuScript U}$.
...
Here $L_{\sf M}$ is the language of the structure $\langle M,F\rangle$.
As both $F$ and $M$ are lattice $\mathds{R}$-algebras, we assume that $L_{\sf M}$ has symbols for these lattice algebras.
...
\begin{proof}
  ...
  If $G$ is bounded, pick for every $\varepsilon\in\mathds{R}^+$ some $\alpha_{\varepsilon,f}\in G_f$ such that 
