...
The following consequence will be useful below

\def\ceq#1#2#3{\parbox[t]{20ex}{$\displaystyle #1$}\parbox{5ex}{\hfil $#2$}{$\displaystyle #3$}}

\begin{definition}[and remark]\label{remk_p'p}
  Let ${\EuScript N}$ be model.
  For $p(x)\subseteq\mathds{I}(N)$, we write \emph{$p'(x)$\/} for the type

  \ceq{\hfill p'(x)}{=}{\big\{\varphi'(x):\varphi'>\varphi\textrm{ for some }\varphi(x)\in p\big\}.}

  By the proposition above, if  ${\EuScript N}$ is $\omega$-$\mathds{I}$-saturated, then $p'(x)$ is equivalent to $p(x)$.
\end{definition}
...
\ceq{\hfill x\sim_Rz}{=}{\Big\{\tau_{t,\Delta}(x,z)\ :\ t(x,y)\textrm{ an } {\EuScript L}\textrm{-terms}\Big\}}

By Remark~\ref{remk_p'p} this is equivalent to

\ceq{ }{\leftrightarrow}{\Big\{\tau_{t,C}(x,z)\ :\ t(z,y)\textrm{ an }{\EuScript L}\textrm{-terms, } C\supseteq\Delta \textrm{ a compact neigborhood}\Big\}}

% \ceq{ }{\leftrightarrow}{\Big\{neg\tau_{t,C}(x,z)\ :\ t(z,y)\textrm{ an }{\EuScript L}\textrm{-terms, } C\supseteq\Delta \textrm{ a compact neigborhood}\Big\}}
...
\begin{proposition}\label{prop_sim_approx}
  ...
  \ceq{\hfill\varphi(x)}{\rightarrow}{\bigwedge_{i=1,\dots,n}\tau_{t_i,C_i}(a,x)\ \ \rightarrow\ \ \varphi'(x)}
\end{proposition}

\begin{proof}
  As $\neg\varphi(x)$ is $\sim_{\!R}$-invariant 

  \ceq{\hfill\neg\varphi(x)}{\rightarrow}{\bigvee_{a\,\models\neg\varphi(x)}\big\{\tau_{t,C}(a,x)\ :\ t(z,y)\textrm{ term, } C\supseteq\Delta \textrm{ compact neigborhood}\big\}.}
  ...
  \ceq{\hfill\varphi(x)}{\leftarrow}{\bigwedge_{a\,\models\neg\varphi(x)}\tau_{t,C}(a,x).}
