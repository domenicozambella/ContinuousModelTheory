...
We will see that, between two saturated models, $\mathds{I}$ and $\mathds{H}$-elementarity coincide with ${\EuScript L}$-elementarity (in particular, they are invertible).

Now we introduce two weaker notions of elementarity.
It will be easy to see that these are invertible in all models.
...
We will see (Proposition~\ref{prop_approx}) that a slight amount of saturation sufficies to make these approximated versions of elementarity redundant.
...
We say that $p(x)$ is a \emph{Cauchy type\/} if it is consistent 
%${\sf R}$-invariant, 
and $p(x)\cup p(y)\rightarrow x\sim_{\sf R}\!y$.
...
Note that for any Cauchy type $p(x)\subseteq\mathds{I}(M)$ the type $\bar{p}(x)=\exists y{\sim_{\sf R}}x\ p(y)$ is an ${\sf R}$-invariant Cauchy type containing $p(x)$.
