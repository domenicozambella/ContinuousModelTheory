...
\begin{abstract}
  In general, very little model theory is possible if we restrict to a class of two-sorted structures $\langle M,R\rangle$ where $M$ is arbitrary and $R$ is fixed.
  But not everything is lost when $R$ is a compact topological space.
  Continuous logic deals with this case.
  Rivisiting ideas of Henson and Iovino we present an approach to continuous logic that is simple and general.
\end{abstract}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
E.g.\@ can we allow in ${\EuScript L}$ function symbols of sort ${\sf M}^n\times {\sf R}^m\to {\sf R}$ with $m,n>0$.
This would be a natural option when $M$ is a group acting on a compact set $R$.
Also symbols of sort ${\sf M}^n\times {\sf R}^m\to {\sf M}$ have natural applications (e.g.\@ consider the action of the field of scalars in a vector space).
...
The approximated $\mathds{H}$-morphisms defined below are the maps that preserve approximated satisfaction.
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Elimination of quantifiers of sort ${\sf R}$}
...
As $C$ ranges over the compact neighborhoods of the diagonal $\Delta\subseteq R^2$ and $t(x,z)$ rangers over the parameter-free terms, the formulas $\tau_{t,C}(x,y)$ define a system of entougages on $U^{|x|}$.
