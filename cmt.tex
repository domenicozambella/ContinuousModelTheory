...
Formulas in ${\EuScript H}$ generalize the positive bounded formulas of Henson and Iovino to a larger class of structures.
...
Alternatively one can view a normed space as a many-sorted structure: a sort for each ball of radius $n\in\mathds{Z}^+$, but this is also a bloated formalism.

A neater formalization is possible if one accepts that some ${\EuScript I}$-elementary extension of a normed space could contain vectors of infinite norm hence not be themselves normed spaces.
...
Note that the same remains true if we add to the language any continuous (a.k.a.\@ bounded) operator or functional.
\end{example}
...
We define \emph{approximate ${\EuScript H}$-elementary\/} maps in the same manner (from Proposition~\ref{prop_LHapprox1} it follows that the two notions coincide).
...
\begin{corollary}\label{corol_st}
  ...
  Let $p(\eta)\subseteq{\EuScript I}(M)$ be a type that does not contain existential quantifiers of sort ${\sf H}$.
 ...
 Let $p(\eta)$ contain the formulas $\exists x\,(x>0\wedge\iota(x)+\eta\in[0,1/n]$ for all positive integers $n$.
...
A consequence of Proposition~\ref{prop_approx} is that the approximate morphisms defined in Section~\ref{morphisms} coincide with their unapproximate version when the codomain is a $\omega$-${\EuScript I}$-saturated model.
...
Let $T$ be a ${\EuScript I}$-theory.

We say that $T$ has (approximate) elimination of quantifiers (of sort ${\sf H}$) if for every $\varphi(x)\in{\EuScript I}$ and every $\varphi'>\varphi$ there is a formula $\psi(x)\in{\EuScript I}$ without quantifiers of sort ${\sf H}$ such that $\varphi(x)\rightarrow\psi(x)\rightarrow\varphi'(x)$ holds modulo $T$.
...
\begin{fact}\label{fact_backandforth}
  ...
  \begin{itemize}
    ...
    \item[2.] every finite partial ${\EuScript I}$-embedding $k:M\to N$ beween models of $T$ is an approximate ${\EuScript I}$-elemenrary map;
    \item[3.]  for every finite partial ${\EuScript I}$-embedding $k:M\to N$ between $\omega$-${\EuScript I}$-saturated models of $T$, and for every $b\in M$ there is a $c\in N$ such that $k\cup\{\langle b,c\rangle\}:M\to N$ is also a partial ${\EuScript I}$-embedding.
  \end{itemize}
\end{fact}
...
The interpretation of $\Pr\varphi(\hat x)$ is 
...
\begin{proof}
  ...
  Let $p(\hat x,\hat z)$ be the ${\EuScript I}$-atomic type of $\hat b,\hat a$ in $M$.
  It suffices to show that $p(\hat x,k\hat a)$ is finitely consistent in $N$.

  We may assume that the formulas in $p(\hat x,\hat z)$ have the form $\Pr\varphi(\hat x,\hat z)=\gamma$ for some $\gamma\in I$.
  Pick a finite set of these formulas, say $\Pr\varphi_i(\hat x,\hat z)=\gamma_i$, for $i<n$.
  By the saturation of $N$, it suffices to prove that for every $\varepsilon$ there is a $\hat c\in N$ such that
  ...
  The consistent formulas among the $\xi_J(z)$ define a partition of ${\EuScript U}^{|z|}$.
  ...
  As the $\varphi_i(x,z)$ define a partition, the $\beta_{i,J}$ add up to $1$ for any fixed $J$ for which they are defined (i.e.\@ such that $\alpha_J\neq0$).
  ...
  Now we prove that the implication (2) \& (4)$\Rightarrow$(1) holds in ${\EuScript U}^\Omega$ when $\Omega$ large enough (larger than $n/\varepsilon$ suffices).
  Strictly speaking, this implication is not a ${\EuScript J}$-formula, but the reader can easily verify that a suitable approximation of (2) and (4) suffices. 
  
  Let $\hat a'\in{\EuScript U}^\Omega$ saisfy (2) and (4) for every $i\in J\subseteq n$. 
  ...
  We may define $\hat c'$ separately in each $\llbracket\xi_J(\hat a')\rrbracket$.
  ...
  Finally, from elementary probability theory (the theorem of total probability)
