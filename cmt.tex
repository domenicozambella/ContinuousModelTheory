...
Hence we may not conclude that $\{\varphi(x)\}'$ is finite.

\begin{proposition}
  Suppose $I=[0,1]$.
  Then for every $\varphi(x)\in{\EuScript I}$ there is a formula $\psi(x)\in{\EuScript I}$ such that $\{\varphi(x)\}'\leftrightarrow\psi(x)$ holds in every model. 
\end{proposition}
...
\ceq{\hfill T}
{=}
{\Big\{\varphi\in{\EuScript L}\ :\ \langle{\EuScript U}^\Omega, I\rangle\models\varphi\textrm{ for every finite set }\Omega\Big\}}
