...
\begin{itemize}
  ...
  Unfortunately, this adds notational burden and restricts the class of structures that can be considered. See~\cite{K} or~\cite{H} for a survey. 
\end{itemize}
...
Let \emph{$I$\/} be some fixed first-order structure which is endowed with a Hausdorff compact\footnote{This seems to exclude the motivation example mentioned above. For this example and many other similar we have to replace $\mathds{R}$ with $\mathds{R}\cup\{\pm\infty\}$.} topology (in particular, a normal topology).
...
\begin{definition}\label{def_0}
  ...
  By \emph{model\/} we mean a two-sorted ${\EuScript L}$-structure of the form $\langle  M,I\rangle$, where $M$ ranges over all $L_{\sf H}$-structures, while $I$ is the structure fixed above.
...
When the map ${\rm id}_M:M\rightarrow N$ is an $\mathds{I}$-elementary embedding, 
% that is, if for every  $\varphi(x)\in\mathds{I}$ and every $a\in M^{|x|}$
%
% \ceq{\hfill{\EuScript M}\models\varphi(a)}{\Rightarrow}{N\models\varphi(a)},
%
we write \emph{$M\preceq^\mathds{I}N$\/} and say that $M$ is an \emph{$\mathds{I}$-elementary submodel\/} of $N$. 
...
We say that $f:M\to N$ is a \emph{partial $\mathds{I}$-embedding\/} if the implication in (1) holds for all $\mathds{I}$-atomic formulas $\varphi(x)$.
...
\begin{proof}
  ...
  By the definition of $\mathds{I}$-embedding, $N\models t(a)\in\tilde{C}$.
  Hence $N\models t(a)\notin C$.
  Induction is immediate.
\end{proof}
...
\begin{theorem}[\boldmath$\mathds{I}$-Compactness Theorem]\label{thm_compattezza}
...
%   Let $p(x\,;y)\subseteq\mathds{I}(M)$ be finitely consistent in $M$, a model.
%   Then $N\models\existsH x\,\exists^I y\,p(x\,;y)$ for some model $N$ such that $M\preceq^\mathds{I}N$.
% \end{corollary}
...
% \begin{definition}
%   We say that $M$ is \emph{$\mathds{I}$-saturated\/} if for every $p(x)$ as in 1 and 2 below, $M\models\existsH x\, p(a)$.
%   \begin{itemize}
%     \item[1.] $p(x)\subseteq\mathds{I}(A)$ for some $A\subseteq  M$ of cardinality $<|M|$ and $|x|=1$;
%     \item[2.] $p(x)$ is finitely satisfied in $M$.
%   \end{itemize}
% \end{definition}
A model $N$ is \emph{$\lambda$-$\mathds{I}$-saturated\/} if it realizes all types $p(x)\subseteq\mathds{I}(N)$ with fewer than $\lambda$ parameters that are finitely consistent in $N$.
When $\lambda=|N|$ we simply say  \emph{$\mathds{I}$-saturated.}
...
%   Let $N$ be saturated and of cardinality larger than $|\mathds{I}|$.
%   Then for every $ab\in M^\alpha$, where $\alpha<|N|$, such that $a\equiv b$, there is an $\mathds{I}$-automorphism of $N$ that maps $a$ to $b$.
% \end{proposition}
...
%   Let $M$ be a model that is $\lambda$-$\mathds{I}$-saturated for some $\lambda\ge|{\EuScript L}|$.
%   Let $f:M\to N$ be an $\mathds{I}$-elementary map.
%   Then
...
%   By saturation there is a model ${\EuScript M_0}\preceq^\mathds{I}M$ that is isomorphic to ${\EuScript N_0}$ over $f$.
...
We denote by \emph{${\EuScript U}$\/} some large $\mathds{I}$-saturated model which we call the \emph{monster model.}
...
We stress once again that the truth of some $\varphi\in\mathds{I}(M)$ in a model $M$ (only) implies the truth of $\varphi$ (in ${\EuScript U}$).
If also the converse implication holds we say $M$ a \emph{strong model.}
By Corollary~\ref{corol_omega_sat}, $\omega$-$\mathds{I}$-saturated models are strong models. 
...
We define a topology on ${\EuScript U}^{|x|}$ which we call the \emph{$\mathds{I}(A)$-topology.}
The closed sets of this topology are the sets defined by the types $p(x)\subseteq\mathds{I}(A)$.
...
% \begin{lemma}
%   For every $\varphi(x\,;y)\in\mathds{I}(A)$ the set $\varphi(U\,;I)$ is a compact subset of ${\EuScript U}^{|x|}\times I^{|y|}$.
% \end{lemma}
...
%   For every $a\in {\EuScript U}^{|x|}$ and $\varphi(x)\in\mathds{H}(U)$, if $\neg\varphi(a)$ then $\tilde\varphi(a)$ for some $\tilde\varphi\perp\varphi$.\hfill\qedsymbol
% \end{corollary}


When $A\subseteq{\EuScript U}$, we write \emph{$S_\mathds{I}(A)$\/} for the set of types 
...
For $a,b\in {\EuScript U}^{|x|}$ we write \emph{$a\equiv_Ab$\/} if $a$ and $b$ satisfy the same ${\EuScript L}$-formulas over $A$, bearing in mind that formulas in $\mathds{H}(A)$ or $\mathds{I}(A)$, or their approximations, suffice to test the equivalence.
...
% Assume $I$ is second countable.
% Fix a countable base of closed sets with nonempty interior.
% Let $Q\subseteq I$ be countable dense subset. 
% Given
...
In Section~\ref{separability} we will prove a version L\"owenheim-Skolem Theorem that is more appropriate to our context.
...
We say that $M$ is \emph{Cauchy complete\/} if every ${\sf I}$-invariant Cauchy type $p(x)\subseteq\mathds{I}(M)$ is realized in $M$.
...
\begin{remark}
  Cauchy complete models are strong models. I.e., $M\models\varphi$ for every true $\varphi\in\mathds{I}(M)$.
\end{remark}


We say that the set $B$ is a \emph{Cauchy closure\/} of $A$ if 
\begin{itemize}
  \item[1.] every ${\sf I}$-invariant Cauchy type $p(x)\subseteq\mathds{I}(A)$ is realized in $B$
  \item[2.] $\mathds{I}\mbox{-tp}(b/A)$ is Cauchy for every $b\in B$.
\end{itemize}
...
\begin{fact}
  Let $M$ be a model that is $\lambda$-$\mathds{I}$-saturated for some $\lambda>|{\EuScript L}|$.
...
\begin{proof}  
  ...
  By Lemma~\ref{lem_Cauchyequiv}, $p(x)\leftrightarrow \sim_{\sf I}\!x$.
...
% Let $|x|=1$.
% It is easy to prove by induction on $\varphi(x,z)\in\mathds{H}$ that if $\varphi(b,a)$ then either $b\in\range a$ or $b\sim_{\sf I}x\to\varphi(x,a)$.

% The proposition is proved by applying Theorem~\ref{prop_Tarski-Vaught}.Assume $\existsH x\ \varphi(x,a)$ and let $b\in U$ be such that $\varphi(b,a)$. 
% By the claim above either 


% and compactness, there is a formula $\psi(x)\in\mathds{I}(M)$ such that  $\psi(x)\to\varphi'(x,a)$.
% As 
...
We say that ${\EuScript U}$ is \emph{separable\/} if there is a countable  ${\EuScript L}_0\subseteq{\EuScript L}$ such that for every $\varphi'>\varphi\in{\EuScript L}$ there is a $\varphi_0\in {\EuScript L}_0$ such that  $\varphi\rightarrow\varphi_0\rightarrow\varphi'$.


\begin{proposition}
  Assume that $I$ is second countable, $L_{\sf H}$ is countable, and there are at most countably many symbols of sort ${\sf H}^n\to{\sf I}$.
...
We prove directly the existence of $\mathds{I}$-saturated extension (monster models), skipping the proof of the $\mathds{I}$-Compactness Theorem as it is not required in the following.
