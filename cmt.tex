...
Though we prove that $\mathds{I}$ has, in a sense, approximatively the same expressive power as $\mathds{H}$ (cf. Propositions~\ref{prop_LHapprox1} and~\ref{prop_LHapprox2}), it is evident that $\mathds{I}$ is more convinent to use.
...
\begin{definition}\label{def_LL}
  ...
  \end{itemize}
  We require that $\mathds{I}$ is closed under the Boolean connectives $\wedge$, $\vee$; the quantifiers $\forall^{\sf M}$, $\exists^{\sf M}$ of sort ${\sf M}$; and the quantifiers of sort ${\sf R}$ which are denoted by $\forall^{\sf R}$, $\exists^{\sf R}$.
...
\begin{example}\label{ex_Rvlogic}
  ...
  \ceq{\hfill\forall^{\sf M}\!x\ \big[t(x)\dotminus\tau\in\{0\}\big]}
  {\wedge}{\forall^{\sf R}\!\varepsilon \Big[\varepsilon\in\{0\}\ \vee\ \exists^{\sf M}\!x\ \big[\tau\dotminus (t(x)+\varepsilon)\in\{0\}\big]\Big]}

  which, in a more legible form, becomes

  \ceq{\hfill\forall^{\sf M}\!x\ \big[t(x)\le\tau\big]}{\wedge}{\forall^{\sf R}\!\varepsilon>0\ \exists^{\sf M}\!x\ \big[\tau\le t(x)+ \varepsilon\big].}
\end{example}
...
This a standard introduction to the sandard part of elements of the nonstandard extension of a compact topological space.
Our goal is to prove Lemma~\ref{lem_st}.
Let $R\preceq{}^*\!R$.
...
% Symbols in $L_{\!R}$ have the interpretation they have in models.
...
\begin{proof}
  ...
  The general case is left to the reader as it follows easily from this special case.

  Assume ${}^*{\!\EuScript M}\models t(a\,;\alpha)\in C$.
...
Moreover the $\mathds{I}$-atomic formulas in $L$ are replaced with their negation and each connective is replaced with its dual i.e., $\vee, \wedge, \exists, \forall, \exists^{\sf R}, \forall^{\sf R}$ are replaced with $\wedge,\vee,\forall,\exists,\forall^{\sf R},\exists^{\sf R}$ respectively.
...
As $\mathds{I}$ and $\mathds{H}$-elementary maps are in particular $L$-elementary, they are injective.
However their inverse need not be $\mathds{I}$, respectively $\mathds{H}$-elementary.
In other words the converse of (1) may not hold.
We will see that $\mathds{I}$ and $\mathds{H}$-elementary are between saturated models they coincide ${\EuScript L}$-elementary, hence the converse of (1) holds trivially.

Now we introduce a two of weak notions of elementarity that are invertible in all models.
We say that $f:M\to N$ is an \emph{$\mathds{I}$-partial embedding\/} if (1) holds when $\varphi(x)$ is $\mathds{I}$-atomic.
...
We say that $f:M\to N$ is \emph{approximately $\mathds{I}$-elementary\/} if for every formula $\varphi(x)\in\mathds{I}$, and every $a\in({\rm dom }f)^{|x|}$
...
We will prove that between saturated model approximately $\mathds{I}$ and $\mathds{H}$-elementary maps are both ${\EuScript L}$-elementary.
In general we have the follwing.
...
%   Then ${\EuScript N}\models\exists^{\sf M}\!x\,\exists^R y\,p(x\,;y)$ for some model ${\EuScript N}$ such that ${\EuScript M}\preceq^\mathds{I}{\EuScript N}$.
% \end{corollary}
...
% \begin{definition}
%   We say that ${\EuScript M}$ is \emph{$\mathds{I}$-saturated\/} if for every $p(x)$ as in 1 and 2 below, ${\EuScript M}\models\exists^{\sf M}\!x\, p(a)$.
%   \begin{itemize}
...
The following fact demostrate how $\mathds{I}$-compactness applies.
...
\begin{proposition}\label{prop_approx}
  For every $\varphi(x)\in\mathds{I}(U)$ 
...
%
  ...
  \ceq{\hfill\bigwedge_{\varphi'>\varphi}\exists^{\sf M}\!z\,\varphi'(x,z\,;y)}
  {\rightarrow}
  {\exists^{\sf M}\!z\,\varphi(x,z\,;y)}

  From (ih) we have

  \ceq{\hfill\exists^{\sf M}\!z\,\bigwedge_{\varphi'>\varphi}\varphi'(x,z\,;y)}
  {\rightarrow}
  {\exists^{\sf M}\!z\,\varphi(x,z\,;y)}
  ...
  \ceq{\hfill\bigwedge_{\varphi'>\varphi}\exists^{\sf M}\!z\,\varphi'(x,z\,;y)}
  {\rightarrow}
  {\exists^{\sf M}\!z\,\bigwedge_{\varphi'>\varphi}\varphi'(x,z\,;y)}

Replace $x,y$ with some fix but arbitrary parameters, say $a,\alpha$ and assume the antecedent, that is, the truth of the theory $\{\exists^{\sf M}\!z\,\varphi'(a,z\,;\alpha):\varphi'>\varphi\}$.
...
%   \ceq{\hfill\varphi}{=}{\exists\varepsilon\big[\varepsilon\in\{0\}\ \wedge\ \exists^{\sf M}\!x,y\ \ {\rm d}(x,y)\cdot\varepsilon\in\{1\}\big]}
...
%  Similarly we define \emph{$S_\mathds{I}(A)$\/} and \emph{$S_{\mathds{I},x}(A)$.}
 ...
 \begin{remark}\label{lem_p'p}
  For $p(x)\subseteq\mathds{I}(U)$, we write \emph{$p'(x)$\/} for the type
...
We show that the quantifiers $\forall^{\sf R}$ and $\exists^{\sf R}$ can be eliminated up to some approximation.
...
\begin{proposition}
  ...
  \begin{itemize}
    ...
    \ceq{\hfill\exists^{\sf M}\!x\,\varphi(x)}{\Rightarrow}
...
\begin{proof}
  (1$\Rightarrow$2) Assume $\exists^{\sf M}\!x\,\varphi(x)$ and let $\varphi'>\varphi$ be given.
  ...
  Then $\neg\forall^{\sf M}\!x\,\tilde{\varphi}(x)$ hence, by (1), ${\EuScript M}\models\neg\forall^{\sf M}\!x\,\varphi(x)$.
  Then ${\EuScript M}\models\neg\tilde{\varphi}(a)$ for some $a\in M$. Hence ${\EuScript M}\models\varphi'(a)$ and $\varphi'(a)$ follows from (1).

  (2$\Rightarrow$3)
  Assume (2) and let $\varphi(x)\in\mathds{I}(M)$ be such that $\exists^{\sf M}\!x\,\neg\varphi(x)$.
  ...
  \ceq{\hfill{\EuScript M}\models\forall^{\sf M}\!y\,\varphi(a,y)}{\Rightarrow}{\forall^{\sf M}\!y\,\varphi(a,y)}.

  Indeed,

  \ceq{\hfill\neg\forall^{\sf M}\!y\,\varphi(a,y)}
  {\Rightarrow}{\exists^{\sf M}\!y\,\neg\varphi(a,y)}
  ...
  \ceq{}
  {\Rightarrow}
  {{\EuScript M}\not\models\forall^{\sf M}\!y\,\varphi(a,y).}

  Induction for the connectives $\vee$, $\wedge$, $\exists^{\sf M}$, $\exists^{\sf R}$, and $\forall^{\sf R}$ is straightforward.
\end{proof}
...
%   The relation $a\sim_{\!R} b$ holds exactly when $a(\omega)=b(\omega)$ for almost every $\omega\in\Omega$.
% \end{example}
...
The relation $x\sim_{\!R} y$ is defined by a parameter-free $\mathds{I}$-type.

  \begin{comment}
\begin{proposition}\label{prop_LHapprox1}
  Let $\varphi(x)\in\mathds{I}(A)$ be $\sim_{\!R}$-invariant.
  For every given $\varphi'>\varphi$ there are some $a_i\models\varphi(x)$ and some terms $t_i(x,y)$ such that 

  \ceq{\hfill\varphi(x)}{\rightarrow}{\bigwedge_{i=1,\dots,n}\forall^{\sf M}\!y\ [t_i(a_i,y)=t_i(x,y)]\ \ \rightarrow\ \ \varphi'(x)}
\end{proposition}

\begin{proof}
  We write $n_a(x)$ for the $\mathds{I}$-type $\big\{\forall^{\sf M}\!y\ [t_i(a,y)=t_i(x,y)]\big\}$
  By Remark~\ref{lem_p'p}

  \ceq{\hfill\neg\varphi(x)}{\rightarrow}{\bigvee_{a\models\varphi(x)}n'_a(x)}
...
\hfil$\forall^{\sf R}\!\varepsilon\ \exists^{\sf R}\!\delta\ \forall^{\sf F}\!f\ \big[{\rm I}\big(|f{\cdot}\mu|\big)<\delta\rightarrow {\rm I}\big(|f{\cdot}\nu|\big)<\varepsilon\big]$.
...
\begin{definition}
  ...
  \item[ii.] It is closed under the Boolean connectives $\bot$, $\top$, $\wedge$, $\vee$, and the quantifiers $\forall^{\sf M}$, $\exists^{\sf M}$ of sort $ M$.
  \item[iii.] It is closed under the quantifier $\forall^{\sf M}$ of sort $\mathds{R}$ relativized to any definable subset of $\mathds{R}$.
  \end{itemize}
\end{definition}
...
\begin{proof}
  ...
  Assume that 3 holds for the formula $\psi(z,y)$ and prove that it holds for the formula $\forall^{\sf M}\!y\,\psi(z,y)$.
  The the implication $\Rightarrow$ is immediate.
  To prove $\Leftarrow$, note that

  \ceq{\hfill{\EuScript N}\models\neg\forall^{\sf M}\!y\ \psi(a,y)}{\Rightarrow}{{\EuScript N}\models\forall^{\sf M}\!y\,{}^{\neg\varepsilon}\psi(a,y)} \hfill for some $\varepsilon>0$

  \ceq{}{\Rightarrow}{{\EuScript M}\models\forall^{\sf M}\!y\,{}^{\neg\varepsilon\kern-.2pt}\psi(a,y)}\hfill for some $\varepsilon>0$

  \ceq{}{\Rightarrow}{{\EuScript M}\models\neg\forall^{\sf M}\!y\ \psi(a,y)}.
...
\begin{proof}
  ...
  \ceq{\hfill p_{\restriction M}(x)}{\rightarrow}{\exists^{\sf M}\!z\ \big[q_{\restriction M}(z)\ \cup\ p'(x,z)\big].}
\end{proof}
...
%   \ceq{\hfill\big\{\varphi(z)\big\}\ \cup\ p_{\restriction C}(z)}{\rightarrow}{\exists^{\sf M}\!x\ \psi(x, z)}
% \end{proof}
...
We write ${\EuScript M}\models\exists^{\sf M}\!x\,\varphi(x)\le0$ if there is an $a\in  M^{|x|}$ such that ${\EuScript M}\models\varphi(a)\le0$.
...
\ceq{\hfill {\EuScript M}\models\bigvee_x\varphi(x)\le0}{\Leftrightarrow}{{\EuScript M}\models\forall^{\sf M}\!x\ \varphi(x)\le0}
...
We write \emph{${\EuScript M}\models \exists^{\sf M}\!x\ p(x)\le0$} if ${\EuScript M}\models p(a)\le0$ for some $a\in  M^{|x|}$.
...
\begin{fact}\label{fact_existential}
  ...
  \begin{itemize}
    \item[1.] $ M\models\exists^{\sf M}\!x\ \big(\varphi(x)\le0\big)$;
