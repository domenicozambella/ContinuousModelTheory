...
Now, we have two conflicting aspirations: we would like to
\begin{itemize}
  \item[i.] apply the basic tools of model theory (such as elementarity and saturation);
  \item[ii.] stay within the realm of normed spaces (hence insist that 
...
The set of the realizations of ${\rm m}_\beta(x)$ is known to nonstandard analysts as the monad of $\beta$. 
...
We show that, if we allow some approximation, the quantifiers $\forall^{\sf R}$ and $\exists^{\sf R}$ can be eliminated.
...
For $a,b\in U$ we write \emph{$a\sim_{\!R} b$\/} if for every parameter free term $t(x,y)$ of sort ${\sf\kern.1em M}^{1+|y|}\to {\sf R}$ it holds $t(a,y)=t(b,y)$. Note that universal quantification over the free variables is understood. 
...
The language ${\EuScript L}$ has also a symbol $d$ for a function of sort ${\sf\kern.1em M}^2\to {\sf R}$.
Let $R=\mathds{R}\cup\{\pm\infty\}$ be the compactification of $\mathds{R}$.
The structure ${\EuScript M}=\langle M,R\rangle$ we interpret the symbols in ${\EuScript L}$ as natural.

Let ${\EuScript U}$ be a monster model that contains ${\EuScript M}$ as a model.
We call the interpretation of $d$ in ${\EuScript U}$ a ${\EuScript U}$-metric.
Clearly this is not a metric, there are distict elements at ${\EuScript U}$-distance $0$, others at infinite distance.
Still, the definition of Cauchy sequence makes perfectly sense for the ${\EuScript U}$-metric and therefore of also the notion of a Cauchy complete set.
...
\begin{proof}
  ...
  By compactness (see Fact~\ref{fact_compactness_imp}) there is a formula $\varphi_i(x)\in p$ such that $\varphi'_i(x)\rightarrow d(a,x)<2^{-i}$ for some $\varphi'_i>\varphi_i$.
...
The relation $x\sim_{\!R} y$ is defined by a parameter-free $\mathds{H}$-type.
...
Let $M$ be the set of $\mathds{R}$-valued measures concentrated on finitely many points of $\Omega$.


Let $M$ be the set of bounded $\sigma$-additive measures on ${\EuScript A}$.
