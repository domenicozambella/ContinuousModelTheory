...
In particular, there is a costant for each element of $R$.
According to the context, $C$ and $f$ denote either the symbols of $L_{\sf R}$ or their interpretation in the stucture $R$.
...
\begin{fact}
  ...
  for every $a\in({\rm dom }f)^{|x|}$ and every $\mathds{I}$-formulas $\varphi(x)$ without quantifiers of sort ${\sf M}$.
\end{fact}
...
%
  ...
  \ceq{\hfill\{\existsM z\,\varphi(x,z\,;y)\}}
  {\rightarrow}
...
Replace $x,y$ with some parameters, say $a,\alpha$ and assume the antecedent, that is, the truth of the theory $\{\existsM z\,\varphi'(a,z\,;\alpha):\varphi'>\varphi\}$.
...
The \emph{weight\/} of ${\EuScript L}$, which we denote by \emph{${\rm wgt}\big({\EuScript L}\big)$\/} is the minimal cardinality of a set of formulas $\mathds{H}'$ such that for every $\varphi,\varphi''\in\mathds{H}$ such that $\varphi''>\varphi$ there is a $\varphi'\in\mathds{H}'$ such that  $\varphi''>\varphi'>\varphi$.
It is not difficult to see that when $R$ is the real unit interval and ${\EuScript L}$ contains at modt countably many symbols of sort ${\sf M}^n\to{\sf R}$ then ${\rm wgt}\big({\EuScript L}\big)=\omega$.
...
For $t(x,z)$ a term of sort ${\sf M}^{|x|+|z|}\to {\sf R}$ and $D\subseteq R^2$ we define the formula

\ceq{\hfill\emph{$x\sim_{t,D}y$}}{=}{\forallM z\ \langle t(x,z),t(y,z)\rangle\in D}

If $t(x,z)=t_1(x,z),\dots,t_n(x,z)$ is a tuple of terms $x\sim_{t,D}y$ denotes the conjunction of the formulas $x\sim_{t_i,D}y$.
As $D$ ranges over the compact neighborhoods of the diagonal $\Delta\subseteq R^2$ and $t(x,z)$ ranges over the finite tuples parameter-free terms, the formulas $x\sim_{t,D}y$ define a system of entougages on $U^{|x|}$.
...
\ceq{\hfill\emph{$x\sim_{\sf R}\!y$}}{=}{\Big\{x\sim_{t,\Delta}y\ :\ t(x,z)\textrm{ parameter-free term}\Big\}.}


\ceq{\hfill\emph{$x\sim'_{\sf R}\!y$}}{=}{\Big\{x\sim_{t,D}y\ :\ t(x,z)\textrm{ parameter-free term, } D\textrm{ compact neigborhood of }\Delta\Big\}.}
...
Let ${\EuScript M}$ be a model.
We say that ${\EuScript M}$ is \emph{Cauchy complete\/} if every ${\sf R}$-invariant Cauchy type $p(x)\subseteq\mathds{I}(M)$ is realized in ${\EuScript M}$.
...
%   Induction is immediate for the connectives $\vee,\forallM,\existsM$.
...
%   Then $\existsM y\sim_{\sf R}\! x\ p(y)$ is an ${\sf R}$-invariant Cauchy type that implies $\varphi$.
...
Let $t(x,y)$ and $D$ be a term of sort ${\sf M}^{|x|+|z|}\to {\sf R}$, respectively a compact neighborhood of the diagonal.
...
Note that this is the classical notion of homogeneity only with the discrete topology replaced by the ${\sf R}$-topology.

We write $\mathds{I}_\forall$ for the set of $\mathds{I}$-formulas where the quantifier $\existsM$ do not occur.
Let $T_0$ be an $\mathds{I}_\forall$-theory with countable models ($L_{\sf M}$ is countable).
Let ${\EuScript C}$ be a category whose objects, which we call ${\EuScript C}$-models, are countable models of $T_0$.
For ease of speaking we identify these models with the domain of their sort ${\sf M}$.
...
Let ${\EuScript M}=\langle M,F,R\rangle$, where $R=\mathds{R}\cup\{\pm\infty\}$.
