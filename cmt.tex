...
We stress once again that for a model ${\EuScript M}$ we (only) have that for every $\varphi\in\mathds{I}(M)$

\ceq{\hfill{\EuScript M}\models\varphi}{\Rightarrow}{{\EuScript U}\models\varphi}
...
We will see that so are Cauchy complete models. 
...
Let $t(x,y)$ and $C$ be a term of sort ${\sf M}^{|x|+|z|}\to {\sf R}$, respectively a compact neighborhood of $\Delta$.
For $\gamma,\xi\in{\rm Aut}({\EuScript U})$ and $a$ a finite tuple of elements of $M$ we write $\gamma\sim_{a,t,C}\xi$ for $\gamma a\sim_{a,t,C} \xi a$.
