...
Let ${\EuScript M}$ be a model.
We say that ${\EuScript M}$ is \emph{Cauchy complete (C.c)\/} if every ${\sf R}$-invariant Cauchy type $p(x)\subseteq\mathds{I}(M)$ is realized in ${\EuScript M}$.
...
\begin{theorem}
  \noindent\llap{\textcolor{red}{\Large\questionsign}\kern15.5ex}\ignorespaces
  Let ${\EuScript M}$ be an e.c.C.c\@ model.
  Then  every consistent formula $\varphi(x)\in\mathds{H}(M)$ is contained in a Cauchy type over $M$.
\end{theorem}
...
The following corollary deals with the case $\varphi(x)$ does not depend on $x$.
...
\begin{proof}  
  ...
  The existential closure of ${\EuScript M}$ follows from Fact~\ref{fact_saturation_invertible}.
\end{proof}
