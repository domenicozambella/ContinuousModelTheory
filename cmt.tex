...
But we use a class of formulas $\mathds{I}$ which is larger than their $\mathds{H}$.
...
\begin{definition}\label{def_0}
  ...
  The new symbols allowed in ${\EuScript L}$ are function symbols of sort ${\sf\kern.1em M}^n\to {\sf R}$. 
\end{definition}
...
\begin{definition}\label{def_LL}
  ...
  \begin{itemize}
  \item[i.] formulas of the form $t(x;y)\in C$, where $C\subseteq R^n$ is compact (in the product topology) and $t(x,y)$ is a $n$-tuple of terms of sort ${\sf\kern.1em M}^{|x|}\times {\sf R}^{|y|}\to {\sf R}$; 
  \item[ii.] all formulas in $L$.
  \end{itemize}
  We require that $\mathds{I}$ is closed under the Boolean connectives $\wedge$, $\vee$; the quantifiers $\forall$, $\exists$ of sort ${\sf\kern.1em M}$; and the quantifiers of sort ${\sf R}$ which are denoted by $\forall^{\sf R}$, $\exists^{\sf R}$.
...
\begin{remark}\label{rk_terms}
  Each component of the tuple $t(x\,;y)$ in the definition above is either of sort ${\sf\kern.1em M}^{|x|}\to {\sf R}$ or of sort ${\sf R}^{|y|}\to {\sf R}$.
  In other words, in each component one of $x$ or $y$ is redundant.
\end{remark}
...
Somewhat surprisingly, we will see (cf.\@ Propositions~\ref{prop_LHapprox1} and~\ref{prop_LHapprox2}) that the formulas in $\mathds{I}$ can be very well approximated by formulas in $\mathds{H}$.

\begin{example}\label{ex_Rvlogic}
  Let $R=[0,1]$, the unit interval in $\mathds{R}$.
  Let $t(x)$ be a term of sort ${\sf\kern.1em M}^{|x|}\to {\sf R}$.
  ...
  {\wedge}{\forall^{\sf R}\varepsilon \Big[\varepsilon\in\{0\}\ \vee\ \exists x\ \big[\tau\dotminus (t(x)+\varepsilon)\in\{0\}\big]\Big]}
...
The set of the realizations ${\rm m}_\beta(x)$ is known to nonstandard analysts as the monad of $\beta$. 
...
\ceq{\hfill{}^*\!R}{\models}{{\rm st}\big(t(\alpha)\big)=t\big({\rm st}(\alpha)\big).}
\end{fact}

\begin{proof}
  We assume that $t$ is a function symbol, say $f$, for some continuous function $f:R^{|x|}\to R$.
  The result for general terms follows.
...
The \emph{standard part of ${}^*{\!\EuScript M}$\/} is the model ${\EuScript M}=\langle M,R\rangle$ that interprets the symbols $f$ of sort ${\sf\kern.1em M}^n\to {\sf R}$ as the functions
...
\begin{fact}\label{fact_terms_st2}
  Let ${}^*{\!\EuScript M}$ and ${\EuScript M}$ be as above.
  For every $a\in M^{|x|}$ and every term $t(x)$ of sort ${\sf\kern.1em M}^{|x|}\to {\sf R}$

  \ceq{\hfill t^{\EuScript M}(a)}{=}{{\rm st}\big(t^{{}^*{\!\EuScript M}}(a)\big)}
\end{fact}
\begin{proof}
  When a $t$ is function symbol $f$, the fact holds by definition.
  For all other terms, it follows from Fact~\ref{fact_terms_st}.
\end{proof}
...
\begin{proof}
  ...
  \begin{itemize}
    \item[1.] Suppose $y$ does not occur in $t(x\,;y)$.  
    ...
    \item[2.] Suppose $x$ does not occur in $t(x\,;y)$.  
  ...
  % If $t$ is a longer tuple of length longer then one.
  % We claim that for every compact set $C_i\subseteq R$ such that $C\subseteq C_1\times\dots\times C_{|y|}$

  % \ceq{\hfill{\EuScript M}}
  % {\models}
  % {t_i(a\,;{\rm st}(\alpha))\in C_i.}

  % By the definition of product topology, $C$ is the intersection of products as those above.
  % Therefore ${\EuScript M}\models t(a\,;{\rm st}(\alpha))\in C$ follows from the claim.
...
We say that $f:M\to N$, a partial map, is an \emph{$\mathds{I}$-elementary map\/} if for every $\varphi(x)\in\mathds{I}$ and every $a\in({\rm dom }f)^{|x|}$

\ceq{\hfill{\EuScript M}\models\varphi(a)}{\Rightarrow}{{\EuScript N}\models\varphi(fa).}

An $\mathds{I}$-elementary map is in particular $L$-elementary and therefore it is injective.
...
\begin{theorem}[$\mathds{I}$-compactness]\label{thm_compattezza}
...
%   Then ${\EuScript N}\models\exists x\,\exists^R y\,p(x\,;y)$ for some model ${\EuScript N}$ such that ${\EuScript M}\preceq^\mathds{I}{\EuScript N}$.
% \end{corollary}
...
% \begin{definition}
%   We say that ${\EuScript M}$ is \emph{$\mathds{I}$-saturated\/} if for every $p(x)$ as in 1 and 2 below, ${\EuScript M}\models\exists x\, p(a)$.
%   \begin{itemize}
...
%   \end{itemize}
% \end{definition}
A model ${\EuScript N}$ is \emph{$\mathds{I}$-saturated\/} if it realizes all types with fewer than $|{\EuScript N}|$ parameters that are finitely consistent in ${\EuScript N}$.
...
Moreover the $\mathds{I}$-atomic formulas in $L$ are replaced with their negation and every connective is replaced with its dual i.e., $\vee, \wedge, \exists, \forall, \exists^{\sf R}, \forall^{\sf R}$ are replaced with $\wedge,\vee,\forall,\exists,\forall^{\sf R},\exists^{\sf R}$ respectively.
...
%
  We consider case of the existential quantifiers of sort ${\sf\kern.1em M}$.
...
%  Similarly we define \emph{$S_\mathds{I}(A)$\/} and \emph{$S_{\mathds{I},x}(A)$.}
 ...
 \begin{corollary}\label{corol_Hcomplete2}
   The types $p(x)\in S_\mathds{H}(A)$ are maximally consistent subsets of $\mathds{H}_x(A)$.
   That is, for every $\varphi(x)\in\mathds{H}(A)$, either $\varphi(x)\in p$ or $p(x)\rightarrow\neg\varphi(x)$.
 \end{corollary}
...
A model ${\EuScript M}$ is \emph{$\mathds{H}$-homogeneous\/} if every $\mathds{H}$-elementary map $f:M\to M$ of cardinality $<|M|$ extends to an automorphism.
...
Still, we can infer an approximate equivalence.
...
\begin{proof}
  ...
  (2$\Rightarrow$3)
  Assume (2) and let $\varphi(x)\in\mathds{I}(M)$ be such that $\exists x\,\neg\varphi(x)$.
  ...
  Induction for the connectives $\vee$, $\wedge$, $\exists$, $\exists^{\sf R}$, and $\forall^{\sf R}$ is straightforward.
\end{proof}
...
For $a,b\in U$ we write \emph{$a\sim_{\!R} b$\/} if $t(a,y)=t(b,y)$ holds (universal quantification over the free variables is understood) for every parameter free term $t(x,y)$ of sort ${\sf\kern.1em M}^{1+|y|}\to {\sf R}$. 
We write $[a]_{\!R}$ for the class of equivalence of $a$.

\begin{fact}
  If $\bar a=\langle a_i:i<\lambda\rangle$ and $\bar b=\langle b_i:i<\lambda\rangle$ are such that $a_i\sim_{\!R} b_i$ for every $i<\lambda$ then $t(\bar a,y)=t(\bar b,y)$ holds for every term $t(x,y)$ of sort ${\sf\kern.1em M}^{\lambda+|y|}\to {\sf R}$ with parameters in $U$.
\end{fact}
...
For $a\in U$ and $A\subseteq U$, let $p(x)=\mathds{H}\mbox{-tp}(a/A)$.
We say that $a$ is a \emph{limit point\/} of $A$ if $p(x)\rightarrow a\sim_{\!R} x$.

\begin{fact}
  Let ${\EuScript M}$ be $\lambda$-saturated for $\lambda>|{\EuScript L}|$.
...
\begin{fact}
  Assume that $R=[0,1]$ and that ${\EuScript L}$ contains a function $d$ of sort ${\sf\kern.1em M}^2\to {\sf R}$ which is interpreted in a pseudometric.
  Assume that all terms of sort ${\sf\kern.1em M}^n\to {\sf R}$ are continuous with rispect to this pseudometric.
  Then the following are equivalent
  \begin{itemize}
    \item[1.] $a\in U$ is definable in the limit over $M$, for some model ${\EuScript M}=\langle M,R\rangle$;
...
\begin{proof}
  ...
  By compactness (see Fact~\ref{fact_compactness_imp}) there is a formula $\varphi_n(x)\in p$ such that $\varphi'_n(x)\rightarrow d(a,x)<1/n$ for some $\varphi'_n>\varphi_n$.
...
The relation $x\sim_{\!R} y$ is defined by an $\mathds{H}$-type (over $\varnothing$).
We say that a formula $\varphi(x)$ is $\sim_{\!R}$-invariant if $x\sim_{\!R} y\rightarrow[\varphi(x)\leftrightarrow\varphi(y)]$.
\end{document}
%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%
...
\begin{example}[Banach spaces]\label{ex_banach}
  ...
  Besides the symbols mentioned above, $\mathds{I}$ contains a function symbol for the natural embedding ${\rm id}: M\to \breve M$.
