...
\begin{definition}\label{def_0}
  ...
  The new symbols allowed in ${\EuScript L}$ are function of sort ${\sf H}^n\to {\sf I}$.
...
As a remedy, below we carve out a set of formulas ${\EuScript I}$, such that $L_{\sf H}\subseteq{\EuScript I}\subseteq {\EuScript L}$ and every model has an ${\EuScript I}$-elementary, ${\EuScript I}$-saturated extension that is also a model.
...
At face value the following fact sounds stronger then it is in reality.
In fact, though we will prove a form of compactness for ${\EuScript I}$-formulas, this is only applicable in Fact~\ref{fact_compactness_imp}.
Hence we may not conclude that $\{\varphi(x)\}'$ is equivalent to some finite subset.

\begin{proposition}
  Suppose $I=[0,1]$.
  Then for every $\varphi(x)\in{\EuScript I}$ there is a formula $\psi(x)\in{\EuScript I}$ such that 
  
  \ceq{\hfill\psi(x)}{\leftrightarrow}{\{\varphi(x)\}'}
...
We say that a partial map $f:M\to N$ is \emph{${\EuScript I}$-elementary\/} if for every $\varphi(x)\in{\EuScript I}$ and every $a\in({\rm dom }f)^{|x|}$

\ceq{1.\hfill M\models\varphi(a)}{\Rightarrow}{N\models\varphi(fa).}

An ${\EuScript I}$-elementary map that is total is called an \emph{${\EuScript I}$-elementary embedding.} 
When the map ${\rm id}_M:M\rightarrow N$ is an ${\EuScript I}$-elementary embedding, 
% that is, if for every  $\varphi(x)\in{\EuScript I}$ and every $a\in M^{|x|}$
%
% \ceq{\hfill{\EuScript M}\models\varphi(a)}{\Rightarrow}{N\models\varphi(a)},
%
we write \emph{$M\preceq^{\EuScript I}N$\/} and say that $M$ is an \emph{${\EuScript I}$-elementary submodel\/} of $N$. 

The definitions of \emph{${\EuScript H}$-elementary\/} map/embedding/submodel is obtained replacing ${\EuScript I}$ by ${\EuScript H}$.

As ${\EuScript I}$ and ${\EuScript H}$-elementary maps are in particular $L_{\sf H}$-elementary, they are injective.
However, their inverse need not be ${\EuScript I}$, respectively ${\EuScript H}$-elementary.
...
We say that the map $f:M\to N$ is an \emph{approximate ${\EuScript I}$-elementary\/} if for every formula $\varphi(x)\in{\EuScript I}$, and every $a\in({\rm dom }f)^{|x|}$

\ceq{\hfill M\models\varphi(a)}{\Rightarrow}{N\models\big\{\varphi(fa)\big\}'.}

We define \emph{approximate ${\EuScript H}$-elementary\/} maps in the same manner.
It is clear that ${\EuScript I}$-elementarity implies its approximate variant, similarly for ${\EuScript H}$-elementarity.
...
\begin{fact}\label{fact_HImorphisms}
  If $f:M\to N$ is approximate ${\EuScript I}$-elementary then
  ...
  The same holds for approximate ${\EuScript H}$-elementary maps.
\end{fact}

\begin{proof}
  $\Rightarrow$ \ Fix $\varphi'>\varphi$ and assume $M\models\varphi'(a)$.
  Let  $\varphi'>\varphi''>\varphi$.
  By the definition of approximate elementarity, $M\models\varphi''(fa)$ and $M\models\varphi'(fa)$ follows.
...
We say that the map $f:M\to N$ is a \emph{partial ${\EuScript I}$-embedding\/} if the implication in (1) holds for all ${\EuScript I}$-atomic formulas $\varphi(x)$.
Note that this is the same as requiring that (1) holds for all formulas without quantifiers of sort ${\sf H}$ (unless they occour in $L_{\sf H}$ formulas).
...
\begin{corollary}\label{corol_st}
  ...
  Let $p(\eta)\subseteq{\EuScript I}(M)$ be a type that does not contain formulas with existential quantifiers of sort ${\sf H}$.
 ...
 The language ${\EuScript L}$ contains the identity map $\iota:M\to I$.
 The type $p(\eta)$ contains the formulas $\exists x\,(x>0\wedge\iota(x)+\eta\in[0,1/n]$ for all positive integers $n$.

% \begin{corollary}\label{corol_Isaturation}
%   Let $M$ be a model and let $p(\eta)\subseteq{\EuScript I}(M)$ be a type that is finitely consistent in $M$, where $\eta$ is a tuple of variables of sort ${\sf I}$.
%   Then $p(\eta)$ is realized in a model $N$ such that $M\preceq^{\EuScript I}N$.
% \end{corollary}
...
\begin{theorem}[Compactness Theorem for \boldmath${\EuScript I}$]\label{thm_compattezza}
  Let $T\subseteq{\EuScript I}$ be finitely consistent.
...
%   Let $p(x\,;y)\subseteq{\EuScript I}(M)$ be finitely consistent in $M$, a model.
%   Then $N\models\existsH x\,\exists^I y\,p(x\,;y)$ for some model $N$ such that $M\preceq^{\EuScript I}N$.
% \end{corollary}
...
% \begin{definition}
%   We say that $M$ is \emph{${\EuScript I}$-saturated\/} if for every $p(x)$ as in 1 and 2 below, $M\models\existsH x\, p(a)$.
%   \begin{itemize}
%     \item[1.] $p(x)\subseteq{\EuScript I}(A)$ for some $A\subseteq  M$ of cardinality $<|M|$ and $|x|=1$;
%     \item[2.] $p(x)$ is finitely satisfied in $M$.
%   \end{itemize}
% \end{definition}
A model $N$ is \emph{$\lambda$-${\EuScript I}$-saturated\/} if it realizes all types $p(x\,;\eta)\subseteq{\EuScript I}(N)$ with fewer than $\lambda$ parameters that are finitely consistent in $N$.
When $\lambda=|N|$ we simply say  \emph{${\EuScript I}$-saturated.}
The existence of ${\EuScript I}$-saturated models is obtained from the classical case just as for Theorem~\ref{thm_compattezza}.

\begin{theorem}
  Every model has an ${\EuScript I}$-elementary extension to a saturated model (possibly of inaccessible cardinality).
\end{theorem}

% Homogeneity follows from saturation by the usual back-and-forth contruction.

% \begin{proposition}(Homogeneity)
%   Let $N$ be saturated and of cardinality larger than $|{\EuScript I}|$.
%   Then for every $ab\in M^\alpha$, where $\alpha<|N|$, such that $a\equiv b$, there is an ${\EuScript I}$-automorphism of $N$ that maps $a$ to $b$.
% \end{proposition}

The following proposition shows that a slight amount of saturation tames the ${\EuScript I}$-formulas.


\begin{proposition}\label{prop_approx}
  Let $N$ be an $\omega$-${\EuScript I}$-saturated model.
  Then 
  
  \ceq{\hfill\big\{\varphi(x\,;\eta)\big\}'}{\leftrightarrow}{\varphi(x\,;\eta)}
  
  holds in $N$ for every formula $\varphi(x,;\alpha)\in{\EuScript I}(N)$.
\end{proposition}
...
\begin{remark}\label{remk_approx_EH_free}
  ...
  In general, some saturation is necessary: consider the model presented after Corollary~\ref{corol_st} and the formula $\exists x\,(x>0\wedge\iota(x)\in\{0\})$. 
\end{remark}
...
%   Let $M$ be a model that is $\lambda$-${\EuScript I}$-saturated for some $\lambda\ge|{\EuScript L}|$.
%   Let $f:M\to N$ be an ${\EuScript I}$-elementary map.
%   Then

%   \ceq{\hfillM\models\varphi(a)}{\Leftrightarrow}{{\EuScript N}\models\varphi(fa)}

%   for every ${\EuScript I}$-atomic formula $\varphi(x)$ and every $a\in({\rm dom }f)^{|x|}$
% \end{fact}
...
%   By saturation there is a model ${\EuScript M_0}\preceq^{\EuScript I}M$ that is isomorphic to ${\EuScript N_0}$ over $f$.
...
We denote by \emph{${\EuScript U}$\/} some large ${\EuScript I}$-saturated model which we call the \emph{monster model.}
...
Below we say \emph{model\/} for ${\EuScript I}$-elementary submodel of ${\EuScript U}$.
We stress once again that the truth of some $\varphi\in{\EuScript I}(M)$ in a model $M$ implies the truth of $\varphi$ (in ${\EuScript U}$) but not vice versa.
...
Hence strong models are those that realize the equivalence in Proposition~\ref{prop_approx}, e.g.\@ $\omega$-${\EuScript I}$-saturated models are strong models.

Let $A\subseteq{\EuScript U}$ be a small set throughout this section.
We define a topology on ${\EuScript U}^{|x|}$ which we call the \emph{${\EuScript I}(A)$-topology.}
The closed sets of this topology are the sets defined by the types $p(x)\subseteq{\EuScript I}(A)$.
This is a compact topology by the Compactness Theorem for ${\EuScript I}$.
The following fact demostrate how ${\EuScript I}$-compactness applies in this context.
There are some subtle differences from the classical setting.
% \begin{lemma}
%   For every $\varphi(x\,;y)\in{\EuScript I}(A)$ the set $\varphi(U\,;I)$ is a compact subset of ${\EuScript U}^{|x|}\times I^{|y|}$.
% \end{lemma}

% \begin{proof}
%   We prove that every finitely intersecting family of closed subsets of $\varphi(U\,;I)$ has nonempty intersection.
%   Without loss of generality we can assume that the sets in this family have the form $\psi(U)\times C_1\times\dots\times C_{|y|}$ for some compact sets $C_i\subseteq I$ and some $\psi(x)\in{\EuScript I}$.
%   Then by ${\EuScript I}$-compactness the intersection of the family is nonempty.
% \end{proof}

\begin{fact}\label{fact_compactness_imp}
  Let $p(x)\subseteq{\EuScript I}(A)$ be a type.
  Then for every  $\varphi(x)\in{\EuScript I}({\EuScript U})$
  \begin{itemize}
...
% \begin{example}
%   The equivalence in the proposition above does not extend to all formulas in ${\EuScript I}$.
...
%   is false, though every $\varphi'>\varphi$ is true (in ${\EuScript M}$ and any ${\EuScript I}$-extension of ${\EuScript M}$).\hfill\qedsymbol
% \end{example}
...
% \begin{corollary}
%   If $\varphi(x),\psi(x)\in{\EuScript I}(U)$ are mutually inconsistent then there are $\varphi'>\varphi$ and $\psi'>\psi$ that are mutually inconsistent.
% \end{corollary}
...
% \begin{fact}
%   Let $\varphi(x)\in{\EuScript I}(A)$, where $A\subseteq M$.
%   Then for every $a\in M^{|x|}$ such that ${\EuScript M}\not\models\varphi(a)$ there is a formula $\tilde\varphi(x)\in{\EuScript I}(A)$ such that ${\EuScript M}\models\tilde\varphi(a)$ and ${\EuScript M}\models\neg\existsH x\,[\tilde\varphi(x)\wedge\varphi(x)]$.
% \end{fact}


% \begin{fact}
%   For every $\varphi(x),\psi(x)\in{\EuScript I}(U)$ such that $\neg\existsH x\,[\varphi(x)\wedge\psi(x)]$ there are $\varphi'>\varphi$ and $\psi'>\psi$ such that $\neg\existsH x\,[\varphi'(x)\wedge\psi'(x)]$.
% \end{fact}
...
When $A\subseteq{\EuScript U}$, we write \emph{$S_{\EuScript I}(A)$\/} for the set of types 

\ceq{\hfill{\EuScript I}\mbox{-tp}(a/A)}{=}{\big\{\varphi(x)\ :\ \varphi(x)\in{\EuScript I}(A)\textrm{ such that }\varphi(a)\big\}}

as $a$ ranges over the tuples of elements of ${\EuScript U}$.
We write  \emph{$S_{{\EuScript I},x}(A)$\/} when the tuple of variables $x$ is fixed.
The same notation applies also with ${\EuScript H}$ for ${\EuScript I}$.

%  Similarly we define \emph{$S_{\EuScript I}(A)$\/} and \emph{$S_{{\EuScript I},x}(A)$.}
...
% \begin{proposition}\label{prop_LHapprox2}
%   Let $\varphi(x)\in{\EuScript I}(A)$. For every given $\tilde{\varphi}\perp\varphi$ there is some $\psi(x)\in{\EuScript H}(A)$ such that $\varphi(x)\rightarrow\psi(x)\rightarrow\neg\tilde{\varphi}(x)$.
% \end{proposition}
...
We say that $p(x)\subseteq{\EuScript I}({\EuScript U})$ is \emph{${\sf I}$-invariant\/} if $x\sim_{\sf I}\!y\rightarrow[p(x)\leftrightarrow p(y)]$.
...
\begin{proposition}\label{prop_perturbation}
  The following are equivalent for every $\varphi(x)\in{\EuScript I}({\EuScript U})$ 
  \begin{itemize}
...
% It is immediate that ${\EuScript I}$-formulas constructed without using (ii) of Definition~\ref{def_LL} are ${\sf I}$-invariant.
...
\begin{lemma}\label{lem_Cauchyequiv}
  ...
  \begin{itemize}
    \item[1.]  $p(x)\subseteq{\EuScript I}(M)$ is an ${\sf I}$-invariant Cauchy type;
...
% % We say that ${\EuScript M}$ is \emph{existentially closed (e.c.)\/} if every consistent conjunction of ${\EuScript I}$-atomic formulas is satsfied in ${\EuScript M}$.
...
%   I.e., $M\models\varphi$ for every true $\varphi\in{\EuScript I}(M)$.
% \end{remark}
...
\begin{proof}
  If $\varphi(x)$ is a conjunction of ${\EuScript I}$-atomic formulas then, as ${\EuScript M}$ is e.c., it is realized by some $a\in M^{|x|}$ hence we can take the type $\{x{=}a,\, \varphi(x)\}$. 
  Induction is immediate for the connectives $\vee,\forallH,\existsH$.
  This suffices because, up to equivalence, we can assume that $\wedge$ only occurs in front of ${\EuScript I}$-atomic formulas.
\end{proof}
...
% and compactness, there is a formula $\psi(x)\in{\EuScript I}(M)$ such that  $\psi(x)\to\varphi'(x,a)$.
% As 
...
\begin{proposition}\label{prop_sim_approx}
  Let $\varphi(x)\in{\EuScript I}(U)$ be ${\sf I}$-invariant.
...
We say that ${\EuScript U}$ has approximate elimination of quantifiers (of sort ${\sf H}$) if for every $\varphi(x)\in{\EuScript I}$ and every $\varphi'>\varphi$ there is a formula $\psi(x)\in{\EuScript I}$ without quantifiers of sort ${\sf H}$ such that $\varphi(x)\rightarrow\psi(x)\rightarrow\varphi'(x)$.
...
The interpretation of $\Pr\big(\varphi(\hat x)\big)$ is obtained by defining
...
\ceq{\hfill\llbracket\varphi(\hat a)\rrbracket}{=}{\big\{\omega\in\Omega\ :\ {\EuScript U}\models\varphi(\omega)\big\}.}

The pseudofinite randomization of $T$ is the ${\EuScript I}$-theory
...
We prove that $T^{\rm pfr}$ is a compete theory with elimination of quantifiers.

\begin{theorem}
  Let $k:M\to N$ be a finite partial ${\EuScript I}$-embedding between $\omega$-${\EuScript I}$-saturated models of $T^{\rm pfr}$.
  Then for every $\hat b\in M$ and there is a $\hat c\in N$ such that $k\cup\{\langle\hat b,\hat c\rangle\}:M\to N$ is an ${\EuScript I}$-partial embedding.
\end{theorem}

\begin{proof}
  Let $k:M\to N$ be as above.
...
We assume that for every $\omega\in\Omega$ the formulas $\varphi_0(x,a_\omega),\dots,\varphi_{n-1}(x,a_\omega)$ are pairwise inconsistent and that their disjunction is a tautology.
Below will show that we can always reduce to this case.

Let $\alpha_i\in I$ be such that

\ceq{\hfill M}{\models}{\alpha_i = \Pr\big(\exists x\,\varphi_i(x,\hat a)\big).}

By the assumption on $k:M\to N$

\ceq{1.\hfill N}{\models}{\alpha_i = \Pr\big(\exists x\,\varphi_i(x,k\hat a)\big)}

Let $\varepsilon>0$ be such that $t(\beta_1,\dots,\beta_n)\in C'$ for every $\beta_1,\dots,\beta_n$ that are at distance $\le\varepsilon$ from $\alpha_1,\dots,\alpha_n$.

We now prove that the following holds modulo $T^{\rm pfr}$ 

\ceq{\hfill\bigwedge_{i=1}^n\eta_i=\Pr\exists x\,\varphi_i(x,\hat z)}
{\rightarrow}
{\exists\hat x\bigwedge_{i=1}^n\eta_i=\Pr\varphi_i(\hat x,\hat z)}

It suffices to prove that (2) holds in ${\EuScript U}^\Omega$ whenever $\#\Omega>1/\varepsilon$.
...
\ceq{\hfill\alpha_s}{=}{\Pr\bigg(\bigwedge_{i<n}\neg^{s_i}\varphi_i(\hat b,\hat a)\bigg).}

Then the following inequality holds in $M$

\ceq{\hfill\alpha_s}{\le}{\Pr\bigg(\exists x\,\bigwedge_{i<n}\neg^{s_i}\varphi_i(x,\hat a)\bigg)}

Therefore the same holds in $N$, with $k\hat a$ for $\hat a$.
We claim that from this inequality it follows that 

\ceq{\hfill\alpha_s}{=_\varepsilon}{\exists\hat x\ \Pr\bigg(\bigwedge_{i<n}\neg^{s_i}\varphi_i(\hat x,\hat a)\bigg).}
...
\ceq{\hfill B_s}{\subseteq}{\bigg\llbracket\exists x\,\bigwedge_{i<n}\neg^{s_i}\varphi_i(x,\hat a)\bigg\rrbracket.}
...
\ceq{1.\hfill\alpha_i}{=}{\Pr\big(\varphi_i(\hat b,\hat a)\big)}

\ceq{3.\hfill\gamma_i}{=}{\Pr\big(\exists x\neg\varphi_i(x,\hat a)\big)}
...
\ceq{\hfill N}{\models}{\exists \hat x\bigwedge\alpha_i\approx_\varepsilon\Pr\big(\varphi_i(\hat b,\hat a)\big)}
...
\ceq{\hfill\varphi(\hat x,\hat z)}{\wedge}{\bigwedge_{i=1}^n t_i\big(\Pr\varphi_1(\hat x,\hat a),\dots,\Pr\varphi_m(\hat x,\hat a)\big)\in C'}
...
\ceq{\hfill t_i(\beta_1,\dots,\beta_m)\in C'_i}{\leftrightarrow}{t_i(h_i/h,\dots,h_m/h)\in C_i} 
...
The set $R$ contains the random variables $X:\Omega\to M$ with finite range (the dependence of $R$ from $M$ will not be displayed in the notation).
The 3-sorted signature ${\EuScript L}$ contains $L_{\sf H}$, $L_{\sf I}$ and for every partitioned formula $\varphi(x\,;z)\in L_{\sf H}$ a function symbol of sort ${\sf R}^{|x|}\times{\sf H}^{|z|}\to I$ which is denoted by $\Pr\varphi(X\,;z)$ and interpreted as the function

\ceq{\hfill X,z}{\mapsto}{\Pr X^{-1}\big[\varphi(M\,;z)\big]}.

The \emph{pseudofinite randomization\/} of $T$ is the theory 

\ceq{\hfill T^R}{=}{\big\{\varphi\in{\EuScript I}\ :\ \langle M,R,I\rangle\models\varphi\textrm{ for all }M\models T\big\}}

\ceq{\hfill\forall x\varphi(x\,;z)}
{\rightarrow }
{\Pr\varphi(X\,;z)=1}

\ceq{\hfill \Pr\varphi(X\,;z)}
{\ge}
{0}

\ceq{\hfill\forall x\neg[\varphi(x\,;z)\wedge\psi(x\,;z)]}
{\rightarrow}
{\Pr[\varphi(X\,;z)\vee\psi(X\,;z)]=\Pr\varphi(X\,;z)+\Pr\psi(X\,;z)}


\ceq{\hfill\exists Y\, \Pr\varphi(X,Y\,;z)}
{=}
{\Pr\exists y\,\varphi(X\,;y,z)}
...
Let $F$ be the set of ${\EuScript I}$-valued ${\EuScript B}$-simple functions, i.e.\@ linear combinations of indicator functions of sets in ${\EuScript B}$.
Let $M$ be the set of ${\EuScript I}$-valued ${\EuScript B}$-measures concentrated on finitely many points of $\Omega$.

Let ${\EuScript M}=\langle M,F,I\rangle$, where $I={\EuScript I}\cup\{\pm\infty\}$.
...
As both $F$ and $M$ are lattice ${\EuScript I}$-algebras, we assume that $L_{\sf MF}$ has symbols for these lattice algebras.
...
Let ${\EuScript U}=\langle {}^*\!M,{}^*\!F,I\rangle$ be a monster model such that ${\EuScript M}\preceq^{\EuScript I}{\EuScript U}$.

\def\ceq#1#2#3{\parbox[t]{25ex}{$\displaystyle #1$}\parbox{5ex}{\hfil $#2$}{$\displaystyle #3$}}

Let $\mu$ be a ${\EuScript I}$-valued ${\EuScript B}$-measure.
By saturation, there is some ${}^*\!\mu\in{}^*\!M$ such that $g\,{\cdot}{}^*\!\mu=\int_\Omega g\,{\rm d}\mu$ for every $g\in F$.

Let $\gamma:\Omega\to{\EuScript I}$. 
...
\begin{proof}
  ...
  If $G$ is bounded, pick for every $\varepsilon\in{\EuScript I}^+$ some $\alpha_{\varepsilon}\in G$ such that 
  ...
  Let $p(y)$ be the ${\EuScript I}$-type that contains, for all $f$ as in the lemma, the formulas 
  ...
  \hfill for all $\varepsilon\in{\EuScript I}^+$\cdot.
...
The ${\EuScript I}$-formula $\forall^{\sf F}\!f\ge0\,\ {\rm I}\big(f{\cdot}\mu^+\big)\ge0$ is obviously true in ${\EuScript M}$ and so is the same formula for $\mu^-$.
...
\begin{proof}
  The following ${\EuScript I}$-formula is true in ${\EuScript M}$.
...
\begin{theorem}[Radon-Nikodym] 
  ...
  Then there is a ${\EuScript B}$-measurable $f:\Omega\to{\EuScript I}$ such that 
  $$
...
\ceq{\hfill a\,\equiv^{\EuScript I}_{\EuScript M}x}
{\rightarrow}
{a\sim_{\EuScript U}x},
...
\begin{proof}
  Let $p(x)={\rm tp}_{\EuScript I}(a/U)$.
...
\clearpage
  ...
  \begin{itemize}
    \item[a.] ${\EuScript I}^n\to{\EuScript I}$
    \item[b.] $ M^n\times{M}^m\to\textrm{any of }  M,\ M,\textrm{ or }{\EuScript I}$
    % \item[c.] $ M^n\times{M}^m\to M$
    % \item[d.] $ M^n\times{M}^m\to{\EuScript I}$
  \end{itemize}
  \item[2.] There is a symbol for every (total) uniformly continuous function ${\EuScript I}^n\to{\EuScript I}$.
  \item[3.] The interpretation of functions symbolsof sort $M^n\to{\EuScript I}$ have \textit{bounded\/} range.
  \item[4.] Every element of $M$ is the image of some term of sort $ M^n\to M$.  
  \item[5.] There is only one unary predicate: $x\le0$, of sort ${\EuScript I}$.
  \end{itemize}
...
\begin{definition}
  We write \emph{$\mathds{T}(A)$\/} for the set of terms of sort $ M^n\times{\EuScript I}^m\to{\EuScript I}$ with parameters in some $A\subseteq  M$.
  Up to equivalence, these terms have the form $f(t(x),y)$, where $t(x)$ is a tuple of terms of sort $M^{|x|}\to{\EuScript I}$, and $f$ is a function ${\EuScript I}^{|t|+|y|}\to{\EuScript I}$.
...
\begin{definition}
  We define the set of formulas \emph{${\EuScript I}(A)$\/} inductively
  \begin{itemize}
  \item[i.] ${\EuScript I}(A)$ contains the atomic formula $t\le0$ for every term $t\in\mathds{T}(A)$.
  ...
  \item[iii.] It is closed under the quantifier $\forall\raisebox{1.1ex}{\scaleto{\sf H}{.8ex}\kern-.2ex}$ of sort ${\EuScript I}$ relativized to any definable subset of ${\EuScript I}$.
  \end{itemize}
\end{definition}

Note that in iii of the definition above, definability is intended with respect to $L(M)$, the full first order language of ${\EuScript M}$, which is strictly larger than ${\EuScript I}(A)$.
In particular, if $\varphi(x,\varepsilon)\in{\EuScript I}(A)$, also $\forall\varepsilon{>}0\;\varphi(x,\varepsilon)\in{\EuScript I}(A)$.

\begin{example}[Banach spaces]\label{ex_banach}
  Given a Banach space  $V$ we define a structure ${\EuScript M}=\langle  M,M,{\EuScript I}\rangle$ as follows.
  Let $M=V$ and let $M=\{a\in M: \|a\|\le1\}$ be the closed unit ball of $V$.
  Besides the symbols mentioned above, ${\EuScript I}$ contains a function symbol for the natural embedding ${\rm id}: M\to M$.
  It also contains a symbol for the norm $\|\mbox-\|:M\to{\EuScript I}$.
  Finally, ${\EuScript I}$ contains the usual symbols of the language of vector spaces.
  These have sort ${M}^n\to M$, for the appropiate $n\in\{0,1,2\}$.

  Terms of sort $M^{|x|}\to{\EuScript I}$ have the form $f(t(x))$ where $f$ is a uniformly continuous function and
...
\begin{example}[Metric spaces of finite diameter]\label{ex_metric}
  Given a metric space $(M,{\rm d})$ of finite diameter we define a structure ${\EuScript M}=\langle  M,{\EuScript I}\rangle$ as follows.
...
%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%
\section{${\EuScript I}$-relations}\label{L-relations}

Equality between elements in the unit ball is not among the formulas that Definition~\ref{def_LL} enumerates in ${\EuScript I}$.
...
We say that $I\subseteq M\times N$ is an \emph{${\EuScript I}$-(elementary) relation\/} between ${\EuScript M}$ and ${\EuScript N}$ (or on ${\EuScript M}$ if the two coicide) if for every $\varphi(x)\in{\EuScript I}$
...
\ \ for every $\varphi(x)\in{\EuScript I}( M)\Big)$,

where $|x|=|a|=|b|=1$.
Note that this relation would be trivial had we included  in ${\EuScript I}(M)$ equality between elements of $M$.
...
\begin{lemma}
  The relation $(\sim_{\EuScript M\!})\subseteq  M^2$ is an ${\EuScript I}$-relation.
  Moreover, it is maximal among the ${\EuScript I}$-relations on ${\EuScript M}$, i.e.\@ no ${\EuScript I}$-relation properly contains $(\sim_{\EuScript M\!})$.
\end{lemma}
...
\begin{proof}
  ...
  Note that $a_i\sim_{\EuScript M\!}b_i$ is equivalent to saying that $\Delta\cup\{(a_i,b_i)\}$ is an ${\EuScript I}$-relation.
  As ${\EuScript I}$-relations are closed under composition $\Delta\cup\big\{(a_1,b_1),\dots,(a_n,b_n)\big\}$ is ${\EuScript I}$-elementary.
  It follows that for every $\varphi(x)\in{\EuScript I}$

  \ceq{2.\hfill{\EuScript M}\models\varphi(a)}{\Leftrightarrow}{{\EuScript M}\models\varphi(b).}
  
  This proves that $(\sim_{\EuScript M\!})$ is an ${\EuScript I}$-relation.
  Finally, maximality is immediate.
  % Suppose $E$ and equivalence ${\EuScript I}$-relation on ${\EuScript M\!}$ properly containing $(\sim_{\EuScript M\!})$.
  % Pick $a,b\in M$ such that $a\mathbin{E}b$ and $a\not\sim_{\EuScript M\!}b$.
\end{proof}


\begin{lemma}
  Let $I\subseteq  M\times N$ be total and surjective ${\EuScript I}$-relation.
  Then there is a unique maximal ${\EuScript I}$-relation containing $I$.
  This maximal ${\EuScript I}$-relation is equal to both $(\sim_{\EuScript M\!})\,I$ and $I\,(\sim_{\EuScript N\!})$, where justapposition of relations stands for composition.
\end{lemma}
\begin{proof}
  It is immediate to verify that $(\sim_{\EuScript M\!})\,I$ is an ${\EuScript I}$-relation containing $I$.
  Let $S$ be any maximal ${\EuScript I}$-relation containing $I$.
...
We write ${\rm Aut}({\EuScript M})$ for the set of maximal, total and surjective, ${\EuScript I}$-relations $I\subseteq  M^2$.
...
% Therefore below we insist in working with ${\EuScript I}$-relations in place of ${\EuScript I}$-elementary maps.
...
An \emph{${\EuScript I}$-(elementary) embedding\/} is an ${\EuScript I}$-relation that is functional, injective and total.

When $\varnothing$ is an ${\EuScript I}$-relation between ${\EuScript M}$ and ${\EuScript N}$ we say that ${\EuScript M}$ and ${\EuScript N}$ are \emph{${\EuScript I}$-(elementary) equivalent\/} and write \emph{${\EuScript M}\equiv^{\EuScript I} {\EuScript N}$.}
When $A\subseteq M\cap N$, we write \emph{${\EuScript M}\equiv_{\!A}{\EuScript N}$\/} if $\Delta_{\!A}$, the diagonal relation on $A$, is an ${\EuScript I}$-relation.
In words, we say that ${\EuScript M}$ and ${\EuScript N}$ are ${\EuScript I}$-equivalent \emph{over $A$.}
Finally, we write \emph{${\EuScript M}\preceq^{\EuScript I}{\EuScript N}$\/} when ${\EuScript M}\subseteq{\EuScript N}$, i.e.\@ ${\EuScript M}$ is a substructure of ${\EuScript N}$, and ${\EuScript M}\equiv_M^{\EuScript I}{\EuScript N}$.
In words, we say that ${\EuScript M}$ is an \emph{${\EuScript I}$-(elementary) substructure\/} of ${\EuScript N}$.


Let $A\subseteq  M$.
The \emph{${\EuScript I}(A)$-topology\/} on $M^{|x|}$ is the topology that has, as a base of closed sets, the ${\EuScript I}(A)$-definable sets.

The ${\EuScript I}(A)$-topology is not T$_0$.
The following lemma asserts that to obtain a Hausdorff topology it suffices to quotient by $\equiv_A^{\EuScript I}$.
...
Note that, if $\varphi(x)\in{\EuScript I}(A)$ then ${}^{\varepsilon\kern-2pt}\varphi(x),\ {}^{\neg\varepsilon\kern-2pt}\varphi(x)\in{\EuScript I}(A)$.
Moreover, the positivity of the formulas in ${\EuScript I}(A)$ ensures that
...
\begin{fact}\label{fact_immediate}
  For every $a\in M^{|x|}$ and very $\varphi(x)\in{\EuScript I}(A)$
...
\begin{lemma}
  If $a\nequiv_A^{\EuScript I}b$ are two elements of $M^{|x|}$ then there are two formulas $\varphi(x), \psi(x)\in{\EuScript I}(A)$ such that $\varphi(a)$ and $\psi(b)$ hold and $\varphi(M)\cap\psi(M)=\varnothing$.
\end{lemma}

\begin{proof}
  Let $\varphi(x)\in{\EuScript I}(A)$ be such that $\varphi(a)$ and $\neg\varphi(b)$.
...
\begin{proposition}[Tarski-Vaught test]\label{prop_Tarski_Vaught} Let ${\EuScript M}\subseteq{\EuScript N}$.
  ...
  Then the following are equivalent
  \begin{itemize}
    \item[1.] ${\EuScript M}\preceq^{\EuScript I}{\EuScript N}$;
    % \item[2.] $M$ is dense in $N$ w.r.t.\@ the ${\EuScript I}(M)$-topology on $N$;
    \item[2.] for every $\varphi(x)\in {\EuScript I}(M)$, if $\varphi(N)\neq\varnothing$, then ${\EuScript N}\models\varphi(b)$ for some $b\in M$.
  \end{itemize}
\end{proposition}
\begin{proof}
  ...
  Induction for the universal quantifier of sort ${\EuScript I}$ is immediate.
\end{proof}
...
When $f$ is bounded, $\lambda\in{\EuScript I}$.

Let $I$ be an infinite set.
Let $\langle {\EuScript M}_i:i\in I\rangle$ be a sequence of structures, say  ${\EuScript M}_i=\langle  M_i,M_i,{\EuScript I}\rangle$, that are \emph{uniformly bounded,} that is, the bounds in 3 of Definition~\ref{def_LL} are the same for all ${\EuScript M}_i$.

Let $F$ be an ultrafilter on $I$.

\begin{definition}\label{def_ultraproduct}
  We define a structure \emph{${\EuScript N}=\langle  N,N,{\EuScript I}\rangle$\/} that we call the \emph{ultraproduct\/} of the models $\langle{\EuScript M}_i:i\in I\rangle$.
  \begin{itemize}
    ...
    \item[5.] If $f$ is a function of sort $ M^n\times{M}^m\to{\EuScript I}$ then 
    $$
...
\begin{fact}
  For every term $t(x)$ of sort $ M^n\to{\EuScript I}$  
...
\begin{proposition}[\L\v{o}\'s Theorem]\label{thm_Los}
  Let ${\EuScript N}$ be as above and let $\varphi(x,y)\in{\EuScript I}(A)$.
  Let $\hat a\in N^{|x|}$.
  Then there is a function $u_{-}:{\EuScript I}^+\to F$ such that for every $\lambda\in{\EuScript I}^{|y|}$
...
\begin{proof}
  ...
  where $t$ is a tuple of terms of sort  $ M^{|x|}\to{\EuScript I}$ and $f$ is a uniformly continuous function ${\EuScript I}^{|t|+|y|}\to{\EuScript I}$. 


  Let $\beta'\in{\EuScript I}^{|t|}$ be such that
  $\displaystyle\lim_{i\to F}\ t(\hat ai)=\beta'$.

  Given $\varepsilon>0$, let $B_\varepsilon$ be a neighborhood of $\beta'$ such that 
  
  \ceq{}{\ }{\big|f(\beta,\,\lambda)-f(\beta',\lambda)\big|<\varepsilon}\hfill for every $\beta\in B_\varepsilon$ and every $\lambda\in{\EuScript I}^{|y|}$.
...
Let $p(x)\subseteq{\EuScript I}(M)$, where $x$ has the sort of the unit ball.
...
\begin{definition}
  We say that ${\EuScript M}$ is \emph{${\EuScript I}$-saturated\/} if for every $p(x)$ as in 1 and 2 below, there is an $a\in  M^{|x|}$ such that ${\EuScript M}\models p(a)$
  \begin{itemize}
    \item[1.] $p(x)\subseteq{\EuScript I}(A)$ for some $A\subseteq  M$ of cardinality $<|M|$ and $|x|=1$;
...
Now we prove that every model embeds ${\EuScript I}$-elementarily in an ${\EuScript I}$-saturated one.
First we prove the following lemma.

\begin{lemma}\label{lem_compattezza}
  Every model ${\EuScript M}$ embeds ${\EuScript I}$-elementarily in a model ${\EuScript N}$ that realizes all types as in the definition above.
\end{lemma}
...
\begin{corollary}\label{thm_compattezza}
  Every model ${\EuScript M}$ is an ${\EuScript I}$-elementary subtructure of some saturated model ${\EuScript N}$ (possibly of inaccessible cardinality).
\end{corollary}

We denote by \emph{${\EuScript U}$\/} some large ${\EuScript I}$-saturated structure which we call the \emph{monster model.}
...
Below we say \emph{model\/} for ${\EuScript I}$-elementariy substructure of ${\EuScript U}$.


\begin{lemma}\label{lem_compactness_implication}
  Let $p(x)\subseteq{\EuScript I}(A)$, where $A\subseteq U$ has small cardinality.
  Let $\varphi(x)\in{\EuScript I}(U)$ be such that $p(x)\rightarrow\varphi(x)$.
...
\ceq{\hfill p_{\restriction M}(x)}{\rightarrow}{p(x):={\rm tp}_{\EuScript I}(a/U)}

or, in other words, if $a'\equiv^{\EuScript I}_Ma$ implies $a'\sim_{\EuScript U}a$.
...
\ceq{\hfill {\rm tp}_{\EuScript I}(a,b/U)}
{=}
{{\rm tp}_{\EuScript I}(a/U)\ \cup\ {\rm tp}_{\EuScript I}(b/U).}
...
We say that a model ${\EuScript N}$ is $M$-${\EuScript I}$-saturated if it realizes all constistent types $p(x)\subseteq{\EuScript I}(M)$.
The following proposition is easy to prove.

\begin{proposition}
  $\bar{\EuScript M}$ is the intersection of all $M$-${\EuScript I}$-saturated models.
\end{proposition}
...
\begin{proof}
  ...
  \ceq{\hfill p'(x,b)}{\rightarrow}{p(x):={\rm tp}_{\EuScript I}(a/U)}
  
  for some $b\in\bar M^{|z|}$ and $p'(x,z)\subseteq{\EuScript I}(M)$ such that $p_{\restriction\bar M}(x)=p'(x,b)$.  
  Let $q(z)={\rm tp}_{\EuScript I}(b/U)$, then
...
\begin{question}
  $\bar{\EuScript M}\preceq_{\EuScript I}{\EuScript U}$ \ ?
\end{question}
% %%%%%%%%%%%%%%%%%%%%%%%
% %%%%%%%%%%%%%%%%%%%%%%%
% %%%%%%%%%%%%%%%%%%%%%%%
...
% Let $a\in {\EuScript U}^{|x|}$.
% We say that ${\EuScript M}$ \emph{defines $a$ in the limit\/} if there is a formula $\varphi(x)\in{\EuScript I}(M)$ and some infinite sets $B_1,\dots,B_n\subseteq M$ such that

% \ceq{\hfill\big\{\varphi(x)\big\}\ \cup\ p_{\restriction C}(x)}{\rightarrow}{p(x):={\rm tp}_{\EuScript I}(a/U)}
...
% \begin{proposition}
%   Suppose that $a'\equiv^{\EuScript I}_Ma$ are both defined in the limit by ${\EuScript M}$.
...
%   For some formula $\varphi(x)\in{\EuScript I}(\bar M)$ and some infinite set $B\subseteq\bar M$ we have that for every infinite $C\subseteq B$
...
%   where $p(x,z)={\rm tp}_{\EuScript I}(a,b/U)$.
%   As $b$ is defined in the limit by ${\EuScript M}$, there is a 

%   \ceq{\hfill\big\{\psi(z)\big\}\ \cup\ q_{\restriction C}(z)}{\rightarrow}{q(z)={\rm tp}_{\EuScript I}(b/U)}
% \end{proof}
...
% \begin{proposition}
%   $\bar{\EuScript M}\preceq^{\EuScript I}{\EuScript U}$.
% \end{proposition}

% \begin{proof}
%   Apply the Tarski-Vaught Test~\ref{prop_Tarski_Vaught}.
%   Let $\psi(x,z)\in{\EuScript I}$ and $a\in\bar M^{|z|}$ be such that $\psi(x,a)$ is consistent.
% Let $p(z)={\rm tp}_{\EuScript I}(a/U)$.
% For some $B\subseteq M$ and some formula $\varphi(z)\in{\EuScript I}(M)$ 
...
\begin{fact}
  Let $I\subseteq{\EuScript U}^2$ be an ${\EuScript I}$-relation of cardinality $<\kappa$.
  Then there is a total and surjective ${\EuScript I}$-relation $S\subseteq{\EuScript U}^2$ containing $I$.
\end{fact}
...
\begin{proof}
  ...
  Let $b\in U$.
  It suffices to prove that there is a $c$ such that $I\cup\{\langle b,c\rangle\}$ is an ${\EuScript I}$-relation.
...
\begin{corollary}
  ...
  Let $p(x)={\rm tp}_{\EuScript I}(a/A)$, where $a\in{\EuScript U}^{|x|}$ is a tuple of length $|x|<\kappa$.
  Then
...
%   Then there is an ${\EuScript I}$-elementary embedding of ${\EuScript M}$ in ${\EuScript N}$.\qed
% \end{corollary}
...
%   Then $I$ is an ${\EuScript I}$-relation.\qed
% \end{fact}
...
We prove directly the existence of ${\EuScript I}$-saturated extension (monster models), skipping the proof of the Compactness Theorem for ${\EuScript I}$ as it is not required in the following.
...
From \L\v{o}\'s Theorem we obtain that every model embeds ${\EuScript I}$-elementarily in a saturated one.
First we prove the following lemma.

\begin{lemma}\label{thm_compattezza}
  Every model ${\EuScript M}$ embeds ${\EuScript I}$-elementarily in a model ${\EuScript N}$ that realizes all types as in 1 and 2.
\end{lemma}
...
\begin{theorem}
  Every model ${\EuScript M}$ embeds ${\EuScript I}$-elementarily in a saturated model.
\end{theorem}
