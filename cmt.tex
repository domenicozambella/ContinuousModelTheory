...
\begin{itemize}
  ...
  \item[2.] Henson and Iovino propose to stick to the standard notion of norm but restrict the notion of elementarity and saturation to a smaller class of formulas (which here is denoted by ${\EuScript H}$).
...
Following Henson and Iovino we also restrict the notions of elementarity and saturation.
But we use a class of formulas ${\EuScript I}$ which is larger than ${\EuScript H}$.
Though ${\EuScript I}$ has approximatively the same expressive power as ${\EuScript H}$ (cf.\@ Propositions~\ref{prop_LHapprox1} and~\ref{prop_LHapprox2}), it is evident (cf.\@ Example~\ref{ex_Rvlogic}) that ${\EuScript I}$ is more convinent to use.
...
As a remedy, below we carve out a set of formulas $L_{\sf H}\subseteq{\EuScript I}\subseteq {\EuScript L}$, such that every model has an ${\EuScript I}$-elementary, ${\EuScript I}$-saturated extension that is also a model.
...
\begin{definition}\label{def_LL}
  Formulas in \emph{${\EuScript I}$\/} are defined inductively from two sorts of \emph{${\EuScript I}$-atomic\/} formulas
  \begin{itemize}
  ...
  Therefore we can safely drop the superscript from quantifiers that are followed by variables.
...
Formulas in ${\EuScript H}$ are a generalization of the positive bounded formulas of Henson and Iovino.
Here we also introduce the larger class ${\EuScript I}$ because it offers some advantages. 
...
In this section we recall the notion of sandard part of an element of the elementary extension of a compact Hausdorff topological space.
Our goal is to prove Lemma~\ref{lem_st} which in turn is required for the proof of the ${\EuScript I}$-Compactness Theorem.
The reader that is willing to accept the ${\EuScript I}$-Compactness Theorem without proof may skip this section, see Theorem~\ref{thm_compattezza}.

Here, instead of reinventing the wiel (i.e. going back to ultrafilters and ultapowers), we assume the classical compacteneess theorem and built on that.
Let $I\preceq{}^*\!I$ be a proper extension.
...
\ceq{\hfill f^M(a)}{=}{{\rm st}\big({}^*\!f(a)\big)}\hfill for all $a\in M^n$,
...
\begin{fact}\label{fact_st2} 
  ...
  \ceq{\hfill t^M\big(a\,;{\rm st}(\alpha)\big)}{=}{{\rm st}\big({}^*t(a\,;\alpha)\big)}
\end{fact}
\begin{proof}
  ...
  \ceq{\hfill t_i^M\big(a\,;{\rm st}(\alpha)\big)}{=}{{\rm st}\big({}^*t_i(a\,;\alpha)\big)}
...
\begin{lemma}\label{lem_st}
  With the notation as above.
  For every $\varphi(x\,;\eta)\in{\EuScript I}$, \  $a\in M^{|x|}$ and $\alpha\in({}^*\!I)^{|\eta|}$ 
  
  \ceq{\hfill\langle M,{}^*\!I\rangle\models\varphi(a\,;\alpha)}
  {\Rightarrow}
...
The approximated ${\EuScript H}$-morphisms we define below are the maps that preserve Henson-Iovino approximated satisfaction.
These morphisms are invertible, cf.\@ Fact~\ref{fact_HImorphisms}.

We say that the map $f:M\to N$ is \emph{approximately ${\EuScript I}$-elementary\/} if for every formula $\varphi(x)\in{\EuScript I}$, and every $a\in({\rm dom }f)^{|x|}$

\ceq{\hfill M\models\big\{\varphi(a)\big\}'}{\Rightarrow}{N\models\big\{\varphi(fa)\big\}'.}

We define \emph{approximately ${\EuScript H}$-elementary\/} maps in the same manner.
We leave to the reader to verify (using Lemma~\ref{lem_interpolation}) that ${\EuScript I}$-elementarity implies its approximated version, similarly for ${\EuScript H}$-elementarity.
...
\begin{theorem}[\boldmath${\EuScript I}$-Compactness Theorem]\label{thm_compattezza}
...
%   \end{itemize}
% \end{definition}
A model $N$ is \emph{$\lambda$-${\EuScript I}$-saturated\/} if it realizes all types $p(x)\subseteq{\EuScript I}(N)$ with fewer than $\lambda$ parameters that are finitely consistent in $N$.
...
\begin{proposition}\label{prop_approx}
  ...
  \ceq{\hfill\big\{\varphi(x)\big\}'}{\leftrightarrow}{\varphi(x)}
  
  holds in $N$ for every formula $\varphi(x)\in{\EuScript I}(N)$.
\end{proposition}
...
By Corollary~\ref{corol_omega_sat}, strong models are those that realize the equivalence in Proposition~\ref{prop_approx}.
Hence $\omega$-${\EuScript I}$-saturated models are strong models.
Also Cauchy complete models, which we introduce below, are strong models.
However, models agree on the approximated truth, see Fact~\ref{fact_HImorphisms}.
...
This is a compact topology by the ${\EuScript I}$-Compactness Theorem~\ref{thm_compattezza}.
...
\begin{fact}\label{fact_compactness_imp}
  ...
  Then for every  $\varphi(x)\in{\EuScript I}(U)$
  \begin{itemize}
...
Clearly, when ${\EuScript U}$ is separable we can improve the proposition above by taking $\lambda=\omega_1$.
...
We prove directly the existence of ${\EuScript I}$-saturated extension (monster models), skipping the proof of the ${\EuScript I}$-Compactness Theorem as it is not required in the following.
