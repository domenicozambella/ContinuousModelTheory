...
We also borrow intuitions and results from nonstandard analisys to an extent that we could also claim as title: nonstandard analysis for the finite logician.
...
In some examples we may need to replace the sort ${\sf M}$  with many sorts ${\sf M}_1\dots,{\sf M}_k$.
In this case the language $L$ governs the structure $\langle{\sf M}_1\dots,{\sf M}_k\rangle$.
The models have the form $\langle{\sf M}_1\dots,{\sf M}_k, R\rangle$.
The language ${\EuScript L}$ has function symbols of sort ${\sf M}_1^{n_1}\dots,{\sf M}_k^{n_k}\to {\sf R}$. 
...
\begin{definition}\label{def_LL}
  ...
  \end{itemize}
  We require that $\mathds{I}$ is closed under the Boolean connectives $\wedge$, $\vee$; the quantifiers $\forall$, $\exists$ of sort ${\sf M}$; and the quantifiers of sort ${\sf R}$ which are denoted by $\forall^{\sf R}$, $\exists^{\sf R}$.
...
\begin{fact}\label{fact_terms_st}
  For every $\alpha\in({}^*\! R)^{|x|}$ and every function symbol of sort ${\sf R}^{|x|}\to{\sf R}$
...
\begin{fact}\label{fact_st2}
  ...
  Then for every $a\in M^{|x|}$ and $\alpha\in{\sf R}^{|y|}$ every term 
...
\begin{proof}
  ...
  holds for the terms $t_1,\dots,t_n$ and let $t=f(t_1,\dots,t_n)$ for some function $f$ of sort ${\sf R}^n\to {\sf R}$.
...
For $a,b\in U$ we write \emph{$a\sim_{\!R} b$\/} if for every parameter free term $t(x,y)$ of sort ${\sf M}^{1+|y|}\to {\sf R}$ it holds $t(a,y)=t(b,y)$. Note that universal quantification over the free variables is understood. 
...
\begin{proof}
  We can assume that both $f$ and $\mu$ are non-negative.
  ...
  % \ceq{\hfill\int_\Omega f\,{\rm d}\mu}{=}{\sup G_f.}
  ...
  If $G_f$ is unbounded, let $\alpha_{\varepsilon,f}=+\infty$.
  ...
  \hfill for all $\varepsilon\in\mathds{R}^+$
...
For simplicity we state it for (nonnegative) ${\EuScript B}$-measures.
